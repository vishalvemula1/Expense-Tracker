...
def handle_integrity_error(e: IntegrityError, context: str = ""):
    ...
    if "key (username)" in error_info and "already exists" in error_info:
        raise IntegrityExceptions.UsernameExists

    if "key (email)" in error_info and "already exists" in error_info:
        raise IntegrityExceptions.EmailExists

    if "uq_category_name_user" in error_info or ("name" in error_info and "user_id" in error_info and "already exists" in error_info):
        raise IntegrityExceptions.CategoryNameExists

    if "violates foreign key constraint" in error_info:
